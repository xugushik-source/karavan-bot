...
T = {
    ...
    "cash_confirmed": {"ru": "👍 Принято — платите наличными по договорённости. Если передумаете, ссылки на оплату выше всё ещё доступны.",
                        ...
                        "hy": "👍 Ընդունված է — վճարեք կանխիկ պայմանավորվածությամբ. Եթե փոխեք միտքը, վերևի հղումները դեռ հասանելի են."},
    "commission_now": {"ru": "💵 Комиссия сервису: {sum} лари\n\nПереводом на счёт:\n{account}\n\nВыберите, как будете платить:",
                        "ka": "💵 საკომისიო სერვისს: {sum} ლარი\n\nგადარიცხვით ანგარიშზე:\n{account}\n\nაირჩიეთ, როგორ გადაიხდით:",
                        "hy": "💵 Ծառայության միջնորդավճար՝ {sum} լարի\n\nՓոխանցումով հաշվին՝\n{account}\n\nԸնտրեք, ինչպես կվճարեք."},
...
def handle_message(message):
    ...
    if chat_type != "private":
        # в группе бот реагирует только на служебное /myid — весь личный онбординг
        # (язык, заполнение объявления и т.д.) не должен идти на виду у всей группы
        if text == "/myid":
    ...
    if text == "/post" and ADMIN_CHAT_ID and chat_id == str(ADMIN_CHAT_ID):
        ...
        if posted and posted.get("ok"):
            tg_send(chat_id, "✅ Плашка опубликована в группе. Рекомендую закрепить её вручную (зажать сообщение → Закрепить).\n\nЕсли хочешь добавить видео-инструкцию — запиши короткий экранкаст (как нажимаешь кнопку → отвечаешь боту → видишь объявление) и просто отправь его в группу отдельным сообщением, я не могу создать такое видео сам.")
        else:
